...
#[test]
fn composite_state_snapshot_restores_through_existing_component_boundaries_when_kvm_is_available() {
    ...
    vcpu.restore_state_snapshot(&reference)
        .expect("composite state restore should succeed");
    let observed = vcpu
        .capture_state_snapshot(&policy)
        .expect("restored composite state capture should succeed");
