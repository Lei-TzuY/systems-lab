...
pub const VIRTIO_BLK_BAR0_GPA: u64 = LONG_MODE_MMIO_DEVICE_GPA;
pub const VIRTIO_BLK_DESCRIPTOR_GPA: u64 = 0x0001_8000;
pub const VIRTIO_BLK_AVAIL_GPA: u64 = 0x0001_8100;
pub const VIRTIO_BLK_USED_GPA: u64 = 0x0001_8200;
pub const VIRTIO_BLK_HEADER_GPA: u64 = 0x0001_8300;
pub const VIRTIO_BLK_DATA_GPA: u64 = 0x0001_8400;
pub const VIRTIO_BLK_STATUS_GPA: u64 = 0x0001_8600;
pub const VIRTIO_BLK_PROOF: &[u8; 4] = b"PBNR";

const VIRTIO_BLK_EXIT_BUDGET: u32 = 40;
...
pub struct VirtioBlkPciGuestResult {
...
pub fn run_virtio_blk_pci_guest(config: VmConfig) -> Result<VirtioBlkPciGuestResult, Error> {
    ...
    let layout =
        LongModeMmioBootLayout::new(memory.region(), image.entry(), LONG_MODE_MMIO_STACK_POINTER)
            .expect("fixed virtio-blk BAR mapping remains valid");
        ...
        |continuation, mmio| {
            ...
            if queue_completion.is_some() {
                return Err(verification_error(
                    "duplicate virtio-blk notify completion barrier",
                ));
            }
                ...
                )
            {
                return Err(verification_error(format!(
                    "unexpected virtio-blk event at notify barrier: {event:?}"
                )));
            }
            let memory = vm
                .guest_memory_mut()
                .ok_or_else(|| verification_error("virtio-blk VM lost registered guest memory"))?;
            let completion = mmio
                .process_virtio_blk_notification(VIRTIO_BLK_BAR0_GPA, memory)
                .map_err(|error| {
                    verification_error(format!("virtio-blk queue processing failed: {error}"))
                })?
                .ok_or_else(|| {
                    verification_error("virtio-blk BAR disappeared before queue processing")
                })?;
    ...
    let completion = queue_completion
        .ok_or_else(|| verification_error("virtio-blk queue was never processed after notify"))?;
    if mmio.take_device_event_record().is_some() {
        return Err(verification_error(
            "virtio-blk execution left an extra unconsumed device event",
        ));
    }
    ...
    let status = mmio
        .virtio_blk_status_at(VIRTIO_BLK_BAR0_GPA)
        .ok_or_else(|| verification_error("virtio-blk status unavailable after execution"))?;
    let driver_features = mmio
        .virtio_blk_driver_features_at(VIRTIO_BLK_BAR0_GPA)
        .ok_or_else(|| {
            verification_error("virtio-blk driver features unavailable after execution")
        })?;
    let queue_enabled = mmio
        .virtio_blk_queue_enabled_at(VIRTIO_BLK_BAR0_GPA)
        .ok_or_else(|| verification_error("virtio-blk queue state unavailable after execution"))?;

    let memory = vm
        .guest_memory()
        .ok_or_else(|| verification_error("virtio-blk VM lost guest memory before verification"))?;
    let used_idx = read_u16(memory, VIRTIO_BLK_USED_GPA + 2)?;
    let used_id = read_u32(memory, VIRTIO_BLK_USED_GPA + 4)?;
    let used_len = read_u32(memory, VIRTIO_BLK_USED_GPA + 8)?;
    ...
    if completion.descriptor_id() != 0
        ...
        || proof.as_slice() != VIRTIO_BLK_PROOF
    {
        return Err(verification_error(format!(
            "virtio-blk verification failed: completion={completion:?}, status={status:#x}, features={driver_features:#x}, queue_enabled={queue_enabled}, used=({used_idx},{used_id},{used_len}), request_status={:#x}, proof={proof:?}",
            request_status[0]
        )));
    }
    ...
    {
        return Err(verification_error(format!(
            "expected virtio-blk HLT at RIP {terminal_rip:#x} with architectural RFLAGS bit1 set, got {report}"
        )));
    }

    Ok(VirtioBlkPciGuestResult {
...
fn validate_io_sequence(exits: &[PortIoExit]) -> Result<(), Error> {
    ...
    if exits.len() != 18 {
        return Err(verification_error(format!(
            "expected 18 port-I/O exits, got {}",
            exits.len()
        )));
    }
    for (cycle, selector) in selectors.into_iter().enumerate() {
        ...
        {
            return Err(verification_error(format!(
                "virtio-blk PCI config cycle {cycle} did not match selector {selector:#010x}"
            )));
        }
    }
    for (exit, expected) in exits[14..].iter().zip(VIRTIO_BLK_PROOF.iter().copied()) {
        ...
        {
            return Err(verification_error(format!(
                "virtio-blk proof output did not match byte {expected:#x}"
            )));
        }
    }
    Ok(())
...
fn validate_mmio_sequence(exits: &[MmioExit]) -> Result<(), Error> {
        ...
        exits.iter().zip(expected).enumerate()
    {
        ...
        {
            return Err(verification_error(format!(
                "virtio-blk MMIO exit {index} mismatch: {exit:?}"
            )));
        }
    }
    Ok(())
...
fn verification_error(detail: impl Into<String>) -> Error {
    Error::HostEnvironment(HostEnvironmentError::VcpuOperation {
        id: VcpuId::BOOT.get(),
        operation: "virtio-blk PCI read proof",
...
fn emit_ring_setup(code: &mut Vec<u8>) {
    emit_movabs(code, 7, VIRTIO_BLK_DESCRIPTOR_GPA); // rdi
...
mod tests {
    ...
    fn fixture_builds_three_descriptor_read_and_terminal_hlt() {
        ...
        assert!(bytes.windows(8).any(|window| window == b"BLK-SECT"));
        assert!(bytes.windows(8).any(|window| window == b"BLKEND!!"));
    }

    #[test]
    fn exit_budget_matches_seven_config_cycles_twenty_one_mmio_four_proof_and_hlt() {
        assert_eq!(VIRTIO_BLK_EXIT_BUDGET, 7 * 2 + 21 + 4 + 1);
    }
}
