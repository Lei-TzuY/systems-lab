...
use toy_tcpip::pcap::{PcapPacket, PcapReader, PcapWriter, LINKTYPE_ETHERNET};
...
fn inspect_packet(idx: usize, pkt: &PcapPacket) {
    ...
    match eth.ethertype {
        EtherType::Arp => {
            match ArpPacket::parse(eth.payload) {
                Ok(arp) => {
                    ...
                    println!(
                        "  │  Sender     : MAC {} | IP {}.{}.{}.{}",
                        arp.sender_mac, arp.sender_ip[0], arp.sender_ip[1], arp.sender_ip[2], arp.sender_ip[3]
                    );
                    println!(
                        "  │  Target     : MAC {} | IP {}.{}.{}.{}",
                        arp.target_mac, arp.target_ip[0], arp.target_ip[1], arp.target_ip[2], arp.target_ip[3]
                    );
                }
        ...
        EtherType::IPv4 => {
            match Ipv4Packet::parse(eth.payload, false) {
                Ok(ip) => {
...
fn run_replay(in_path: &str, out_path: &str) {
    ...
    stack.udp_sockets.bind(7, |_src_ip, _src_port, payload| {
        Some(payload.to_vec())
    });
    ...
    println!("Replaying packets from '{}' into NetStack (IP: {}, MAC: {})...", in_path, stack_ip, stack_mac);
...
fn run_demo() {
    ...
    println!("   - Sender IP      : {}.{}.{}.{}", resp_arp.sender_ip[0], resp_arp.sender_ip[1], resp_arp.sender_ip[2], resp_arp.sender_ip[3]);
    println!("   - ARP Cache Updated: 192.168.1.100 -> {}", stack.arp_table.lookup(&client_ip.0).unwrap());
    ...
    let ip_req = Ipv4Packet::serialize(client_ip, stack_ip, toy_tcpip::ipv4::IP_PROTO_ICMP, 101, 64, &icmp_req);
    let eth_req = EthernetFrame::serialize(stack_mac, client_mac, toy_tcpip::ethernet::ETHERTYPE_IPV4, &ip_req);

    println!("Client sends ICMP Echo Request (Ping, ID=0x1337, Seq=1, \"Hello NetStack ICMP Echo!\")");
    ...
    println!("   - Type / Code    : {} / {}", ping_icmp.icmp_type, ping_icmp.code);
    println!("   - Identifier     : 0x{:04x}", ping_icmp.identifier);
    println!("   - Sequence Number: {}", ping_icmp.sequence_number);
    println!("   - Payload Match  : \"{}\"", String::from_utf8_lossy(ping_icmp.payload));
    ...
    let ip_udp_req = Ipv4Packet::serialize(client_ip, stack_ip, toy_tcpip::ipv4::IP_PROTO_UDP, 102, 64, &udp_req);
    let eth_udp_req = EthernetFrame::serialize(stack_mac, client_mac, toy_tcpip::ethernet::ETHERTYPE_IPV4, &ip_udp_req);

    println!("Client sends UDP to port 7 (length {} bytes)", udp_data.len());
    ...
    let udp_resp = UdpDatagram::parse(udp_ip.header.src_ip, udp_ip.header.dst_ip, udp_ip.payload, true).unwrap();
    println!("⚡ NetStack replied with UDP Echo response:");
    println!("   - Src Port / Dst Port: {} -> {}", udp_resp.src_port, udp_resp.dst_port);
    println!("   - Echoed Payload     : \"{}\"", String::from_utf8_lossy(udp_resp.payload));
    ...
    let ip_syn = Ipv4Packet::serialize(client_ip, stack_ip, toy_tcpip::ipv4::IP_PROTO_TCP, 103, 64, &syn_seg);
    let eth_syn = EthernetFrame::serialize(stack_mac, client_mac, toy_tcpip::ethernet::ETHERTYPE_IPV4, &ip_syn);
    ...
    println!("3️⃣  [Client -> Server] TCP ACK (Seq={}, Ack={})", syn_ack_tcp.ack_num, server_isn + 1);
    ...
    let ip_ack = Ipv4Packet::serialize(client_ip, stack_ip, toy_tcpip::ipv4::IP_PROTO_TCP, 104, 64, &ack_seg);
    let eth_ack = EthernetFrame::serialize(stack_mac, client_mac, toy_tcpip::ethernet::ETHERTYPE_IPV4, &ip_ack);
    let _ = stack.process_frame(&eth_ack);

    let key = toy_tcpip::tcp::TcpConnectionKey {
        local: toy_tcpip::tcp::SocketAddrV4 { ip: stack_ip, port: 80 },
        remote: toy_tcpip::tcp::SocketAddrV4 { ip: client_ip, port: client_port },
    };
    ...
    println!("4️⃣  [Client -> Server] TCP PUSH DATA ({} bytes: \"GET /index.html ...\")", http_req.len());
    let data_seg = TcpSegment::serialize(
        ...
        syn_ack_tcp.ack_num,
        server_isn + 1,
        toy_tcpip::tcp::TcpFlags { psh: true, ack: true, ..Default::default() },
        65535,
        http_req,
    );
    let ip_data = Ipv4Packet::serialize(client_ip, stack_ip, toy_tcpip::ipv4::IP_PROTO_TCP, 105, 64, &data_seg);
    let eth_data = EthernetFrame::serialize(stack_mac, client_mac, toy_tcpip::ethernet::ETHERTYPE_IPV4, &ip_data);
    ...
    let ack_resp_tcp = TcpSegment::parse(ack_resp_ip.header.src_ip, ack_resp_ip.header.dst_ip, ack_resp_ip.payload, true).unwrap();
    println!("5️⃣  [Server -> Client] TCP ACK for Data (Ack={})", ack_resp_tcp.ack_num);

    let conn_after = stack.tcp_manager.connections.get(&key).unwrap();
    println!("📥 NetStack received stream payload: \"{}\"", String::from_utf8_lossy(&conn_after.rx_buffer));
...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "inspect" => {
            if args.len() < 3 {
                eprintln!("Error: 'inspect' requires a PCAP file path: toy_tcpip inspect <file.pcap>");
                process::exit(1);
            }
            run_inspect(&args[2]);
        }
        "replay" => {
            if args.len() < 4 {
                eprintln!("Error: 'replay' requires input and output PCAP paths: toy_tcpip replay <input.pcap> <output.pcap>");
                process::exit(1);
            }
        ...
        other => {
            ...
            eprintln!("Available commands: demo, shell, inspect <file.pcap>, replay <in.pcap> <out.pcap>");
            process::exit(1);
        }
    }
}
