use toy_tcpip::gnmi::{
    GNMI_PORT, GNMI_VERSION, GnmiPath, GnmiServer, GnmiSubscriptionMode, GnmiValue,
};

#[test]
fn test_gnmi_path_parsing_and_conversion() {
    let path =
        GnmiPath::parse("/interfaces/interface[name=HundredGigE0/1]/state/counters/in-octets");
...
fn test_gnmi_get_and_set_datastore() {
    ...
    server.set_value(
        "/system/config/motd",
        GnmiValue::StringVal("Welcome to Router Core".to_string()),
    );
...
fn test_gnmi_subscribe_mode() {
    ...
    let (mode, updates) =
        server.handle_subscribe("/system/state/hostname", GnmiSubscriptionMode::Stream);
