...
fn test_nat_udp_dns_translation_and_reverse() {
    ...
    let parsed_udp_out =
        UdpDatagram::parse(public_ip, dns_server_ip, parsed_out.payload, true).unwrap();
    ...
    let mut reply_ip =
        Ipv4Packet::serialize(dns_server_ip, public_ip, IP_PROTO_UDP, 200, 64, &reply_udp);
    ...
    let parsed_udp_in =
        UdpDatagram::parse(dns_server_ip, client_ip, parsed_in.payload, true).unwrap();
