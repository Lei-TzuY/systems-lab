use toy_tcpip::sip::{build_simple_sdp, SipMessage, SipMethod, SIP_PORT};

#[test]
fn test_sip_invite_sdp_and_ok_response() {
    let sdp = build_simple_sdp("alice", "192.168.1.100", 4000);
    let invite = SipMessage::build_invite("alice@example.com", "bob@example.com", "call-abc-123", &sdp);
    ...
    assert_eq!(parsed_ok.is_response, true);
