...
use toy_tcpip::evpn_igmp_join_suppress::{
    EvpnIgmpJoinSuppressEngine, JoinSuppressVerdict,
};
...
fn test_evpn_igmp_join_suppress_integration() {
    ...
    for i in 1..=10 {
        ...
        if i == 1 {
            assert!(matches!(v, JoinSuppressVerdict::FirstSubscriberProxyJoin { .. }));
        } else {
            assert!(matches!(v, JoinSuppressVerdict::DuplicateJoinSuppressed { .. }));
        }
    }
