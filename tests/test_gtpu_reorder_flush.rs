...
use toy_tcpip::gtpu_reorder_flush::{
    GtpuReorderFlushEngine, GtpuReorderFlushVerdict,
};
...
fn test_gtpu_reorder_flush_integration() {
    ...
    assert!(matches!(v1, GtpuReorderFlushVerdict::InOrderPacketEmitted { seq_number: 1, .. }));

    let v3 = engine.ingest_packet(3, 1000, 200);
    assert!(matches!(v3, GtpuReorderFlushVerdict::PacketBuffered { seq_number: 3, buffer_depth: 1, .. }));

    let v4 = engine.ingest_packet(4, 1000, 300);
    assert!(matches!(v4, GtpuReorderFlushVerdict::PacketBuffered { seq_number: 4, buffer_depth: 2, .. }));

    let v5 = engine.ingest_packet(5, 1000, 400);
    assert!(matches!(v5, GtpuReorderFlushVerdict::PacketBuffered { seq_number: 5, buffer_depth: 3, .. }));
