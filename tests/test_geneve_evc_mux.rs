use toy_tcpip::geneve_evc_mux::{
    EvcServiceProfile, EvcServiceType, EvcVlanDeliveryAction, GeneveEvcEngine,
    GENEVE_OPT_CLASS_CARRIER_ETHERNET, GENEVE_OPT_TYPE_EVC_METADATA,
};
...
fn test_geneve_evc_service_multiplexing_and_vlan_strip() {
    ...
    let encap10 = engine.encapsulate_evc_frame("ge-0/0/0", &customer_frame_vlan10).unwrap();
    assert_eq!(encap10.remote_vtep, Ipv4Address::new(172, 16, 0, 1));
    assert_eq!(encap10.geneve_packet.vni, 10010);
    assert_eq!(encap10.geneve_packet.options[0].class, GENEVE_OPT_CLASS_CARRIER_ETHERNET);
    assert_eq!(encap10.geneve_packet.options[0].opt_type, GENEVE_OPT_TYPE_EVC_METADATA);

    // Egress delivery on ge-0/0/1 strips the VLAN tag
    let decap10 = engine.decapsulate_evc_packet(&encap10.geneve_packet).unwrap();
    assert_eq!(decap10.out_if, "ge-0/0/1");
    assert_eq!(decap10.evc_id, 10);
    assert_eq!(decap10.customer_frame.len(), customer_frame_vlan10.len() - 4);
