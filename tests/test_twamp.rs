use toy_tcpip::twamp::{
    calculate_twamp_metrics, TwampServerGreeting, TwampTestPacket, TWAMP_CONTROL_PORT, TWAMP_MODE_AUTHENTICATED,
    TWAMP_MODE_ENCRYPTED, TWAMP_MODE_UNAUTHENTICATED, TWAMP_TEST_PORT,
};
...
fn test_twamp_constants_and_greeting() {
    ...
    assert_eq!(parsed.modes, TWAMP_MODE_UNAUTHENTICATED | TWAMP_MODE_AUTHENTICATED);
...
fn test_twamp_test_packet_and_metrics_calculation() {
    ...
    assert_eq!(raw_resp.len() >= 41, true);
    ...
    let metrics = calculate_twamp_metrics(t1_sec, t1_frac, t2_sec, t2_frac, t3_sec, t3_frac, t4_sec, t4_frac);
    assert_eq!(metrics.rtt_us > 0.0, true);
    assert_eq!(metrics.forward_delay_us > 0.0, true);
    assert_eq!(metrics.reverse_delay_us > 0.0, true);
}
